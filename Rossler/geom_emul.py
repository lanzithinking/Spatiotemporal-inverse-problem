...
def geom(parameter,bip,emulator,geom_ord=[0],**kwargs):
    ...
    def ll_f(x):
        ...
        if bip.misfit.STlik:
            dif_obs = tf.reshape(dif_obs,[-1,bip.misfit.obs.size])
            W = tf.convert_to_tensor(bip.misfit.stgp.tomat(),dtype=tf.float32)
            ll = -0.5*tf.math.reduce_sum(dif_obs*tf.transpose(tf.linalg.solve(W,tf.transpose(dif_obs))),axis=1)
        else:
            ll = -0.5*tf.math.reduce_sum(dif_obs**2/bip.misfit.nzvar,axis=1)
        return ll
