...
for m in range(n_mdl):
    ...
    if found:
        ...
        X=loaded['X']
        ensbls=pd.DataFrame(np.exp(X[np.random.RandomState(2021).choice(np.arange(ensbl_sz,X.shape[0]),size=num_samp,replace=False)]),columns=['a','b','c'])
        
        # form the data frame
        mdl_array=np.array([mdl_names[m]]*num_samp)
        df_samps=pd.DataFrame(samps[m].reshape((-1,input_dim)),columns=['a','b','c'])
