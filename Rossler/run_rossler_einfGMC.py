"""
Main function to run (inf-)geometric MCMC for Rossler inverse problem
Shiwei Lan @ ASU, 2022
"""
...
def main(seed=2021):
    ...
    parser.add_argument('step_sizes', nargs='?', type=float, default=[3.5,.01,.02,None,None]) # .0001 for simple likelihood model
    ...
    rsl = Rossler(num_traj=num_traj, obs_times=obs_times, avg_traj=avg_traj, var_out=var_out, seed=seed, STlik=STlik) # set STlik=False for simple likelihood; STlik has to be used with avg_traj
...
#     x_test,y_test=X[n_tr:],Y[n_tr:]
    ...
    # initialization
    u0=rsl.prior.sample()
    emul_geom=lambda q,geom_ord=[0],**kwargs:geom(q,rsl,emulator,geom_ord,**kwargs)
    
    # run MCMC to generate samples
    print("Preparing %s sampler with step size %g for %d step(s)..."
          % (args.algs[args.algNO],args.step_sizes[args.algNO],args.step_nums[args.algNO]))
    
    e_infGMC=einfGMC(u0,rsl,emul_geom,args.step_sizes[args.algNO],args.step_nums[args.algNO],args.algs[args.algNO])#,k=5) # uncomment for manifold algorithms
    ...
    filename=os.path.join(e_infGMC.savepath,'Rossler_'+{True:'avg',False:'full','aug':'avgaug'}[rsl.misfit.avg_traj]+'_'+e_infGMC.filename+'_'+args.emus[args.emuNO]+'.pckl') # change filename
