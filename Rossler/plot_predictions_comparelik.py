"""
Plot prediction of spatiotemporal observations in Rossler inverse problem.
...
rsl_pred = Rossler(num_traj=num_traj, obs_times=pred_times, avg_traj=avg_traj, var_out=var_out, seed=seed,
                   use_saved_obs=False, save_obs=False)

# obtain true trajectories
true_trj=rsl_pred.misfit.observe().squeeze()
# selective locations to aggregate difference in observations
cond =[t<=1100 for t in rsl_pred.misfit.obs_times]
...
# store results
pred_m=np.zeros((num_mdls,num_algs,num_ensbls,rsl_pred.x[STATE].shape[-1],len(rsl_pred.misfit.obs_times)))
pred_std=np.zeros((num_mdls,num_algs,num_ensbls,rsl_pred.x[STATE].shape[-1],len(rsl_pred.misfit.obs_times)))
err_m=np.zeros((num_mdls,num_algs,num_ensbls,len(rsl_pred.misfit.obs_times)))
err_std=np.zeros((num_mdls,num_algs,num_ensbls,len(rsl_pred.misfit.obs_times)))
# obtain predictions
folder = './analysis'
                    ...
                    if '_'+algs[i]+'_ensbl'+str(ensbl_szs[j])+'_' in f_i:
                        try:
                            ...
                            for s in range(ensbl_szs[j]):
                                ...
                                rsl_pred.x[PARAMETER]=np.exp(params[s]);
                                rsl_pred.x[STATE]=rsl_pred.ode.solveFwd(params=rsl_pred.x[PARAMETER], t=rsl_pred.misfit.obs_times)[0]
                                pred=rsl_pred.misfit.observe(sol=rsl_pred.x[STATE]).squeeze()
...
num_rows=2
j=2
for k in range(rsl_pred.x[STATE].shape[-1]):
    ...
    for ii,ax in enumerate(axes.flat):
        ...
        ax.plot(rsl_pred.misfit.obs_times,true_trj[:,k],color='red',linewidth=1.5)
        ax.plot(rsl_pred.misfit.obs_times,pred_m[m][i][j][k],linestyle='--')
        ax.fill_between(rsl_pred.misfit.obs_times,pred_m[m][i][j][k]-1.96*pred_std[m][i][j][k],pred_m[m][i][j][k]+1.96*pred_std[m][i][j][k],color='b',alpha=.1)
...
for ii,ax in enumerate(axes.flat):
    ...
    #     ax.plot(rsl_pred.misfit.obs_times,cred_cover,linestyle='--')
    ...
    # # elif ii==1:
    # #     ax.plot(rsl_pred.misfit.obs_times,np.zeros(len(rsl_pred.misfit.obs_times)),color='red',linewidth=1.5)
    # #     ax.plot(rsl_pred.misfit.obs_times,err_m[0][i][j],linestyle='--')
    # #     ax.fill_between(rsl_pred.misfit.obs_times,err_m[0][i][j]-1.96*err_std[0][i][j],err_m[0][i][j]+1.96*err_std[0][i][j],color='b',alpha=.1)
    # #     ax.plot(rsl_pred.misfit.obs_times,err_m[1][i][j],linestyle='-.')
    # #     ax.fill_between(rsl_pred.misfit.obs_times,err_m[1][i][j]-1.96*err_std[1][i][j],err_m[1][i][j]+1.96*err_std[1][i][j],color='y',alpha=.1)
    # else:
    #     # m=ii%2; k=locs[m]
    #     k=ii-1
    k=ii
    ax.plot(rsl_pred.misfit.obs_times,true_trj[:,k],color='red')
    ax.plot(rsl_pred.misfit.obs_times,pred_m[0][i][j][k],linestyle='--')
    ax.fill_between(rsl_pred.misfit.obs_times,pred_m[0][i][j][k]-1.96*pred_std[0][i][j][k],pred_m[0][i][j][k]+1.96*pred_std[0][i][j][k],color='b',alpha=.1)
    ax.plot(rsl_pred.misfit.obs_times,pred_m[1][i][j][k],linestyle='-.',color='g')
    ax.fill_between(rsl_pred.misfit.obs_times,pred_m[1][i][j][k]-1.96*pred_std[1][i][j][k],pred_m[1][i][j][k]+1.96*pred_std[1][i][j][k],color='g',alpha=.2)
    ax.set_xlabel('t', fontsize=16)
