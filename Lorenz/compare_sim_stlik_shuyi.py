...
from nn.dnn_rnn_shuyi import DNN_RNN
...
train_sizes = [100,500,1000,5000,10000]#[50,100,200,500,1000]
...
df_err=pd.DataFrame({'algorithm':alg_array.flatten(),
                     'training size':trs_array.flatten(),
                     'function error':fun_err_array.flatten(),
                     'relative function error':remfun_err_array.flatten(),
                     'log relative error':np.log(remfun_err_array).flatten(),
                     'training time':train_time_array.flatten(),
#                      'testing_time':test_time_array.flatten()
                     })

alg_array=np.hstack((['DNN']*n_seed*n_train,['DNN_RNN']*n_seed*n_train,['time-average']*n_seed*n_train,['STGP']*n_seed*n_train))
...
df_time=pd.DataFrame({'algorithm':alg_array.flatten(),
                      'training size':trs_array.flatten(),
                      'testing time':test_time_array.flatten()
                     })
...
sns.barplot(x='training size',y='function error',hue='algorithm',data=df_err,errwidth=1,capsize=.1)
...
plt.axes(axes)
sns.set(font_scale=1.1)
sns.barplot(x='training size',y='log relative error',hue='algorithm',data=df_err,errwidth=1,capsize=.1)
plt.gca().legend().set_title('')
plt.savefig(os.path.join(folder,'log_rem_sim_stlik.png'),bbox_inches='tight')
...
sns.pointplot(x='training size',y='training time',hue='algorithm',data=df_err,ci=None)
# plt.title('Training Time')
plt.gca().legend().set_title('')
plt.axes(axes.flat[1])
sns.pointplot(x='training size',y='testing time',hue='algorithm',data=df_time,ci=None)
