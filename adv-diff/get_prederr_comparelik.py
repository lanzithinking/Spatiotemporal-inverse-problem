"""
Get relative error of forward prediction in Advection-Diffusion inverse problem.
Shiwei Lan @ U of Warwick, 2016
-------------------------------
Modified for STIP October 2021 @ ASU
"""
        ...
        for f_i in hdf5_files:
            if '_'+algs[i]+'_' in f_i:
                try:
                    ...
                    fwderr_mean=0; #fwderr_std=0; #num_read=0
...
#                             u=u_
                        ...
                        # err=np.linalg.norm(pred[np.ix_(obs_idx,slab_idx)]-true_trj[np.ix_(obs_idx,slab_idx)])
                        # fwderr_mean+=wts[num_read][s]*err**2
                        fwderr_mean+=wts[num_read][s]*pred
                        # fwderr_std+=wts[num_read][s]*err**2
                    # compute error
                    # errs.append(np.sqrt(fwderr_mean)/np.linalg.norm(true_trj[np.ix_(obs_idx,slab_idx)]))
                    errs.append(np.linalg.norm(fwderr_mean[np.ix_(obs_idx,slab_idx)]-true_trj[np.ix_(obs_idx,slab_idx)])/np.linalg.norm(true_trj[np.ix_(obs_idx,slab_idx)]))
                    num_read+=1
                    f.close()
                    print(f_i+' has been read!')
                    f_read=f_i
...
# save
import pandas as pd
err_m = pd.DataFrame(data=err_m,index=lik_mdls,columns=alg_names[:num_algs])
err_s = pd.DataFrame(data=err_s,index=lik_mdls,columns=alg_names[:num_algs])
err_m.to_csv(os.path.join(folder,'prederr-mean.csv'),columns=alg_names[:num_algs])
err_s.to_csv(os.path.join(folder,'prederr-std.csv'),columns=alg_names[:num_algs])