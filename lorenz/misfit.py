        ...
        if self.STlik:
            ...
            if self.stgp is None:
                ...
                if self.STlik in (True,'sep'):
                    ...
                    # C_x=GP(self.obs.mean(axis=0).T, l=.4, sigma2=np.diag(np.sqrt(sigma2_)), store_eig=True, jit=1e-2)
                    ...
                    self.stgp=GP(self.obs.mean(axis=0).flatten(), l=.4, sigma2=sigma2, store_eig=True, jit=1e-3)
                else:
