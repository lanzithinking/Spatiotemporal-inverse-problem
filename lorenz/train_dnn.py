...
W = tf.convert_to_tensor(lrz.misfit.nzvar[0],dtype=tf.float32) # lrz.misfit.nzvar[0]
# loglik = lambda y: -0.5*tf.math.reduce_sum((y-lrz.misfit.obs)*tf.linalg.solve(W,(y-lrz.misfit.obs)[:,:,None])[:,:,0],axis=1)
...
# sub_figs=[None]*2
#
# true_input=list(lrz.misfit.true_params.values())
# dim=[0,1]
# levels=20
# grad=True
#
...
#     # Z=np.reshape([lrz.get_geom(u)[0] for u in Input], X.shape) if i==0 else logLik(Input).numpy().reshape(X.shape)
#     Z=np.reshape(Parallel(n_jobs=n_jobs)(delayed(lrz.get_geom)(u)[0] for u in Input), X.shape) if i==0 else logLik(Input).numpy().reshape(X.shape)
#     if grad:
...
#         # G=np.array([lrz.get_geom(u,geom_ord=[0,1])[1] for u in Input]) if i==0 else dnn.gradient(Input, logLik)
#         G=np.array(Parallel(n_jobs=n_jobs)(delayed(lrz.get_geom)(u,geom_ord=[0,1])[1] for u in Input)) if i==0 else dnn.gradient(Input, logLik)
