...
fld=os.getcwd()
try:
    f=open(os.path.join(fld,'Lorenz_obs_'+{True:'avg',False:'full','aug':'avgaug'}[avg_traj]+'_traj_'+{True:'nzvar',False:'','cov':'nzcov'}[var_out]+'.pckl'),'rb')
...
for i,ax in enumerate(axes.flat):
    plt.axes(ax)
    plt.plot({0:np.arange(obs.shape[1])*0.4,1:np.arange(obs.shape[0])*0.1}[i], ACF[i].T, alpha=.4)
    plt.plot({0:np.arange(obs.shape[1])*0.4,1:np.arange(obs.shape[0])*0.1}[i], ACF[i].mean(0), color='r',linewidth=2)
