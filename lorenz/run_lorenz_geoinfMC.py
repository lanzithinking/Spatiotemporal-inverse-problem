...
def main(seed=2021):
    ...
    parser.add_argument('step_sizes', nargs='?', type=float, default=[.0001,.005,.004,None,None]) # [.001,.005,.005] simple likelihood model
    ...
    # initialization
    unknown=lrz.prior.sample(add_mean=True)
    ...
    inf_GMC.geom=lambda parameter: lrz.get_geom(np.insert(parameter,0,np.log(lrz.misfit.true_params['sigma'])),geom_ord=[0])
    lrz.prior.sample = lambda add_mean=True: lrz.prior.std[1:]*np.random.randn(2)+(lrz.prior.mean[1:] if add_mean else 0)
