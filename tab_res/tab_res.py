...
# Lorenz dynamical inverse problem
# relative error of mean in posterior estimates
try:
    rem_m = pd.read_csv('../Lorenz/analysis/REM-mean.csv')
    rem_s = pd.read_csv('../Lorenz/analysis/REM-std.csv')
    sumry = rem_m
    ...
    sumry_tab = sumry.to_latex(index=False, escape=False)
    # print(sumry_tab)
    f = open('../Lorenz/analysis/comparelik.tex', 'w')
    f.write(sumry_tab)
    f.close()
except Exception as e:
    print(e)
...
# Chen dynamical inverse problem
# relative error of mean in posterior estimates
try:
    rem_m = pd.read_csv('../Chen/analysis/REM-mean.csv')
    rem_s = pd.read_csv('../Chen/analysis/REM-std.csv')
    sumry = rem_m
    fmt = lambda x: "%.2e" % x if abs(x)<.01 else "%.2f" % x
    for i in range(sumry.shape[0]):
        for j in range(1,sumry.shape[1]):
    ...
    sumry_tab = sumry.to_latex(index=False, escape=False)
    # print(sumry_tab)
    f = open('../Chen/analysis/comparelik.tex', 'w')
    f.write(sumry_tab)
    f.close()
except Exception as e:
    print(e)