...
if __name__=='__main__':
    ...
    var_out = 'cov'
    STlik = 'sep'
    chn = Chen(num_traj=num_traj, ode_params=ode_params, obs_times=obs_times, avg_traj=avg_traj, var_out=var_out, seed=seed, STlik=STlik)
    ...
    g.map_diag(plot_pdf, para0=para0, f=lambda param:chn._get_misfit(parameter=np.log(param)))
    g.map_lower(contour, para0=para0, f=lambda param:np.exp(-chn._get_misfit(parameter=np.log(param))), cmap='gray')
