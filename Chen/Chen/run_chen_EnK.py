...
def main(seed=2021):
    ...
    rsl = Chen(num_traj=num_traj, prior_params=prior_params, obs_times=obs_times, avg_traj=avg_traj, var_out=var_out, seed=seed, STlik=STlik) # set STlik=False for simple likelihood; STlik has to be used with avg_traj
    ...
    filename=os.path.join(savepath,'Chen_'+{True:'avg',False:'full','aug':'avgaug'}[rsl.misfit.avg_traj]+'_'+filename+'.pckl') # change filename
