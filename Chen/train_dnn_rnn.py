...
chn = Chen(num_traj=num_traj, prior_params=prior_params, obs_times=obs_times, avg_traj=avg_traj, var_out=var_out, seed=seed, STlik=STlik)
...
W = tf.convert_to_tensor(chn.misfit.stgp.tomat(),dtype=tf.float32) # chn.misfit.stgp.tomat()
...
for n in range(n_dif):
    ...
    # ll_xact -= chn.misfit.stgp.logdet()/2
    ll_xact = -chn._get_misfit(u,option='quad')
    dll_xact = -chn._get_grad(u)
